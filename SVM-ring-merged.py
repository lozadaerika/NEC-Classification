...
# Load the datasets
file_path = 'A2-ring/A2-ring-merged-normalized.csv'
...
svmclass.printPlots(df_dataset,'Ring_merged')
...
try:
    for kernel in svm_kernels:
        for constant in range(svm_constant_min, svm_constant_max +1 , svm_constant_step):
            for degree in range(svm_degree_min, svm_degree_max +1 , svm_degree_step):
                print(f"PARAM> kernel={kernel} C={constant}","Degree",degree)
                accuracy, E=svmclass.svm_classification(X_train,X_test,y_train, y_test,kernel,constant,degree,'Ring_merged',False)
    ...
    print("Best parameters:",best_kernel,best_constant,best_degree, best_accuracy, best_error)
    svmclass.svm_classification(X_train,X_test,y_train, y_test,best_kernel,best_constant,best_degree,'Ring_merged',True)
